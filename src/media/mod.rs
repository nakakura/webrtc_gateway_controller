...
pub async fn create_media(
    ...
    let parser =
        |r: reqwest::Response| r.json::<formats::CreateMediaResponse>().map_err(Into::into);
...
pub async fn create_rtcp(base_url: &str) -> Result<CreateRtcpResponse, error::ErrorEnum> {
    ...
    let parser = |r: reqwest::Response| r.json::<formats::CreateRtcpResponse>().map_err(Into::into);
    common::api_access(reqwest::StatusCode::CREATED, false, api_call, parser).await
}
...
pub async fn create_call(
    ...
    let parser = |r: reqwest::Response| r.json::<formats::CallResponse>().map_err(Into::into);
...
pub async fn answer(
    ...
    let parser = |r: reqwest::Response| r.json::<formats::AnswerResponse>().map_err(Into::into);
...
pub async fn events(
    ...
    let parser = |r: reqwest::Response| {
        r.json::<formats::MediaConnectionEventEnum>()
            .map_err(Into::into)
    };
    match common::api_access(reqwest::StatusCode::OK, true, api_call, parser).await {
        ...
        Err(e) => match e {
            error::ErrorEnum::MyError { error: message } if message == "recv RequestTimeout" => {
                Ok(formats::MediaConnectionEventEnum::TIMEOUT)
            }
            e => Err(e),
        },
    }
...
pub async fn status(
    ...
    let api_call = || Client::new().get(&api_url).send();
    let parser = |r: reqwest::Response| {
        r.json::<formats::MediaConnectionStatus>()
            .map_err(Into::into)
    };
